...
const { userLoading, user, showMessage } = storeToRefs(useUserStore());
...
configure({
  generateMessage: localize("en", {
    messages: {
      ...
      min: "The {field} value must be more than 6 chars",
      confirmed: "The {field} value must match the password",
    },
  }),
});
...
var resetPart = url.split("reset/?token=")[1].split("&source")[0];

request.token = resetPart;
...
<template>
  <div class="screen-container">
    ...
    <div class="right-side">
      <Form class="form-section" @submit="resetHandler" v-slot="{ errors }">
        ...
        <transition name="bounce" v-if="showMessage">
          <div class="bg-success/10 rounded-lg p-4 text-center mb-4 mt-4">
            <p class="text-success text-sm font-primary font-medium">
              {{ user?.msg }}
            </p>
          </div>
        </transition>
        ...
        <div class="flex justify-between">
          <p class="point">Should not be less than 6 chrs.</p>
          <ErrorMessage
        ...
        <div class="flex justify-between">
          <p class="point">Should match the new password field.</p>
          <ErrorMessage
            class="text-sm text-red-600 capitalize"
            name="confirmPassword"
          />
...
.logo {
  @apply mb-[170px] lg:block md:block hidden sm:block;
}
.left-side {
...
.input-div {
  @apply bg-white  flex items-center border-2 py-2 px-3  rounded-sm;
}
...
.point {
  @apply font-primary text-[11px] text-[#5E5E5E] font-normal  mb-4;
}
...
.error-text {
  @apply text-red-600 uppercase text-sm font-primary font-medium;
}
