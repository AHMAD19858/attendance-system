<template>
  <nav class="fixed w-[calc(100%)] pe-2 z-20 bg-white">
    <div class="flex h-14 items-center border-b border-[#E6E5E4]">
      <div
        class="aside-nav bg-white transition-all h-full border-r border-[#AEACA8]"
        v-if="user?.role?.title == 'Manager'"
      >
        <!-- logo -->
        <div class="relative flex justify-between px-3 items-center py-2">
          <svg
            @click="toggleSidebar()"
            v-if="sidebarOpen"
            class="h-[60px] w-auto max-w-full cursor-pointer"
            width="64"
            height="64"
            viewBox="0 0 64 64"
            fill="#000"
          ...
          <svg
            v-else
            @click="toggleSidebar()"
            class="h-[90px] w-auto max-w-full cursor-pointer"
            width="64"
            height="64"
            viewBox="0 0 64 64"
            fill="#000"
      ...
      >
        <HeadlessMenuButton
          type="button"
          class="flex items-center gap-1.5 xl:gap-2"
        >
          ...
          <div>
            <p
              class="text-[#4C4C4C] hidden lg:block font-medium font-primary text-sm text-start"
            >
              {{ user?.first_name }} {{ user?.last_name }}
            </p>
            <p
              class="font-primary font-light hidden lg:block text-[13px] text-[#6E7A84]"
            >
              {{ user?.job_title }}
            </p>
          </div>
        ...
        >
          <HeadlessMenuItems
            as="ul"
            class="absolute right-0 mt-2 w-48 rounded-md bg-white dark:bg-[#282828] shadow-2xl shadow-zinc-300 ring-1 ring-black/5 dark:shadow-zinc-800 focus:outline-none dark:border dark:border-[#ffffff1a]"
          >
